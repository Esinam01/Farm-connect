...
const SEV = {
  critical: { color: "#ef4444", bg: "#fef2f2", border: "#fecaca", label: "Critical", icon: "warning"                   },
  high:     { color: "#f97316", bg: "#fff7ed", border: "#fed7aa", label: "High",     icon: "alert-circle"               },
  medium:   { color: "#eab308", bg: "#fefce8", border: "#fef08a", label: "Medium",   icon: "alert-outline"              },
  low:      { color: "#3b82f6", bg: "#eff6ff", border: "#bfdbfe", label: "Low",      icon: "information-circle-outline" },
};
...
function StopAlertsBanner({ onStop }) {
...
export default function FarmAlertSystem() {
  ...
  const doClearAll = () =>
    Alert.alert("Clear History", "Remove all non-active alerts?", [
      { text: "Cancel", style: "cancel" },
      { text: "Clear", style: "destructive", onPress: () => setAlerts(p => p.filter(a => a.status === "active")) },
    ]);
  ...
  const doCall = contact =>
    Alert.alert(`Call ${contact.name}?`, contact.number, [
      { text: "Cancel", style: "cancel" },
      { text: "Call", onPress: () => Linking.openURL(`tel:${contact.number}`).catch(() => Alert.alert("Error", "Cannot place call.")) },
    ]);

  const doSMS = (contact, alertObj) => {
    const body = encodeURIComponent(`🚨 FARM ALERT\nZone: ${alertObj?.zone ?? "Unknown"}\nSeverity: ${(alertObj?.severity ?? "").toUpperCase()}\n${alertObj?.message ?? ""}\nTime: ${alertObj ? fmtTime(alertObj.timestamp) : ""}`);
  ...
  const showStopBtn    = activeAlerts.length > 0 && !alertsSilenced;
        ...
        <View style={styles.sensorGrid}>
          {SENSOR_ZONES.slice(0, 4).map(s => (
            <View key={s.id} style={[styles.sensorChip, s.status === "alert" && styles.sensorChipAlert, s.status === "offline" && styles.sensorChipOffline]}>
              <Ionicons name={s.status === "online" ? "wifi" : s.status === "alert" ? "warning" : "wifi-outline"} size={13} color={s.status === "online" ? "#10b981" : s.status === "alert" ? "#ef4444" : "#9ca3af"} />
      ...
      <ScrollView style={styles.tabBody} showsVerticalScrollIndicator={false}>
        {filteredAlerts.length === 0 ? (
          <View style={styles.emptyBig}>
            ...
            <Text style={styles.emptyBigSub}>{alertFilter === "active" ? "Farm is secure — no active threats." : "Switch filters to see other alerts."}</Text>
          </View>
    ...
    <ScrollView style={styles.tabBody} showsVerticalScrollIndicator={false}>
      ...
      {SENSOR_ZONES.map(s => (
        <View key={s.id} style={styles.sensorRow}>
          <View style={[styles.sensorStatusDot, { backgroundColor: s.status === "online" ? "#10b981" : s.status === "alert" ? "#ef4444" : "#d1d5db" }]} />
          <View style={{ flex: 1 }}>
            <Text style={styles.sensorRowName}>{s.zone}</Text>
            <Text style={styles.sensorRowTime}>Last checked: {s.lastCheck}</Text>
          </View>
          <View style={[styles.sensorStatusBadge, { backgroundColor: s.status === "online" ? "#f0fdf4" : s.status === "alert" ? "#fef2f2" : "#f9fafb" }]}>
            <Text style={[styles.sensorStatusBadgeText, { color: s.status === "online" ? "#10b981" : s.status === "alert" ? "#ef4444" : "#9ca3af" }]}>{s.status}</Text>
          </View>
          {s.status === "alert" && (
  ...
  return (
    <View style={styles.root}>
      ...
      <View style={styles.header}>
        <TouchableOpacity onPress={() => { VibManager.stop(); router.back(); }} style={styles.backBtn}>
      ...
      {criticalCount > 0 && (
        <View style={styles.critBanner}>
          <PulseDot color="#fff" />
          <Text style={styles.critBannerText}>{criticalCount} CRITICAL — immediate action required</Text>
          <TouchableOpacity onPress={() => { setTab("alerts"); setAlertFilter("active"); }}>
            <Text style={styles.critBannerLink}>View →</Text>
          </TouchableOpacity>
        </View>
      ...
      {showStopBtn && <StopAlertsBanner onStop={handleStopAll} />}
          ...
          <View style={styles.modalSheet}>
            ...
            <View style={styles.modalHeaderRow}>
              ...
              <View style={{ flex: 1 }}>
                ...
                {callAlert && <Text style={styles.modalSub}>{callAlert.zone} · {SEV[callAlert.severity]?.label} alert</Text>}
              </View>
            ...
            <ScrollView style={{ paddingHorizontal: 20 }} showsVerticalScrollIndicator={false}>
              {callAlert && (
                <View style={[styles.alertRecap, { backgroundColor: SEV[callAlert.severity]?.bg, borderColor: SEV[callAlert.severity]?.border }]}>
                  <Text style={[styles.alertRecapTitle, { color: SEV[callAlert.severity]?.color }]}>{SEV[callAlert.severity]?.label} Alert — {callAlert.zone}</Text>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalSheet}>
            ...
            <ScrollView style={{ paddingHorizontal: 20 }} showsVerticalScrollIndicator={false}>
              ...
              <TouchableOpacity style={styles.triggerBtn} onPress={() => { injectAlert(simZone, simSev, "manual"); setSimVisible(false); setTab("alerts"); setAlertFilter("active"); }}>
                <Ionicons name="warning" size={18} color="#fff" />
                <Text style={styles.triggerBtnText}>Trigger Alert Now</Text>
              </TouchableOpacity>
              <Text style={styles.simNote}>Simulates a sensor firing. In production, your IoT hardware sends this automatically over WiFi.</Text>
              <View style={{ height: 32 }} />
            </ScrollView>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  tabItem:                { flexDirection: "row", alignItems: "center", paddingHorizontal: 14, paddingVertical: 8, borderRadius: 20, gap: 5 },
